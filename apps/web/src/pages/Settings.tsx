...
export default function Settings() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--color-bg-2)' }}>
      <StatusBar />
      <PushHeader title="설정" onBack={() => navigate(-1)} />
      ...
      <div style={{ padding: '8px 22px 40px' }}>
        ...
        >
          ...
          <span style={{ flex: 1, textAlign: 'left' }}>
            <span style={{ display: 'block', fontSize: 15.5, fontWeight: 800, color: 'var(--color-text)' }}>{user?.nickname ?? '프로필'}</span>
            <span style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--color-text-subtle)' }}>@{user?.username} · 프로필 편집</span>
          </span>
          <Chevron />
        </button>

        {/* 테마 색상 */}
        <div style={{ fontSize: 13, fontWeight: 800, color: '#7C8AA0', margin: '0 0 11px 4px' }}>테마 색상</div>
        <div style={{ background: 'var(--color-card)', borderRadius: 22, padding: '20px 18px', boxShadow: '0 6px 20px rgba(17,40,86,.06)', marginBottom: 22 }}>
          <div role="radiogroup" aria-label="테마 색상" className="flex justify-between">
            {THEME_NAMES.map((name) => {
              ...
              return (
                ...
                >
                  ...
                  <span style={{ fontSize: 11, fontWeight: selected ? 800 : 600, color: selected ? 'var(--color-text)' : 'var(--color-text-subtle)' }}>{meta.label}</span>
                </button>
              )
            })}
          </div>
          ...
          <div className="flex" style={{ gap: 9, marginTop: 20, paddingTop: 18, borderTop: '1px solid #F0F3F8' }}>
            <div className="flex items-center justify-center" style={{ flex: 1, height: 42, borderRadius: 14, background: 'var(--color-primary-strong)', color: '#fff', fontSize: 13, fontWeight: 800 }}>미리보기</div>
            <div className="flex items-center" style={{ height: 42, padding: '0 16px', borderRadius: 14, background: 'var(--color-primary-tint)', color: 'var(--color-primary-strong)', fontSize: 13, fontWeight: 800 }}>배지</div>
        ...
        {/* 환경설정 */}
        <div style={{ fontSize: 13, fontWeight: 800, color: '#7C8AA0', margin: '0 0 11px 4px' }}>환경설정</div>
        <div style={{ background: 'var(--color-card)', borderRadius: 22, padding: '2px 18px', boxShadow: '0 6px 20px rgba(17,40,86,.06)', marginBottom: 22 }}>
          <div className="flex items-center justify-between" style={{ padding: '15px 0', borderBottom: '1px solid #F0F3F8' }}>
            <span className="flex items-center" style={{ gap: 12 }}>
              ...
              <span style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--color-text)' }}>다크 모드</span>
            </span>
            <Toggle on={darkMode} onClick={toggleDark} label="다크 모드" />
          </div>
          <button
            type="button"
          ...
          >
            <span className="flex items-center" style={{ gap: 12 }}>
              ...
              <span style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--color-text)' }}>알림 설정</span>
            </span>
            <Chevron />
          </button>
        </div>
        ...
        {/* 계정 group */}
        <div style={{ fontSize: 13, fontWeight: 800, color: '#7C8AA0', margin: '0 0 11px 4px' }}>계정</div>
        <div style={{ background: 'var(--color-card)', borderRadius: 22, padding: '2px 18px', boxShadow: '0 6px 20px rgba(17,40,86,.06)', marginBottom: 22 }}>
          <button
            type="button"
            onClick={() => navigate('/settings/account')}
          ...
          >
            <span style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--color-text)' }}>계정</span>
            <Chevron />
          </button>
          <div className="flex items-center justify-between" style={{ padding: '15px 0', borderBottom: '1px solid #F0F3F8' }}>
            <span style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--color-text)' }}>언어</span>
            <span className="flex items-center" style={{ gap: 8 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-text-subtle)' }}>한국어</span>
            </span>
          </div>
          <button
            type="button"
            ...
            style={{ padding: '15px 0' }}
          >
            <span style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--color-text)' }}>도움말</span>
            <Chevron />
          </button>
        </div>
